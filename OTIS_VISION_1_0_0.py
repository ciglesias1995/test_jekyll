...
class AimenWorker(QThread):
    ...
    def __init__(self, plc_siemens,plc_aimen, db_number=6):
        super().__init__()
        self.plc_siemens = plc_siemens
        self.db_number = db_number
        self._running = True
        self.simulacion = True  # Cambiar a False para datos reales
    ...
    def run(self):
        while self._running:
            ...
            if self.simulacion:
                ...
                try:
                    if not self.plc_aimen.get_connected(): continue
                    data = self.plc_aimen.db_read(self.db_number, 0, 32)
                    clin_altura_tramo_0 = get_int(data, 0)
                    clin_altura_tramo_1 = get_int(data, 2)
                    clin_altura_tramo_2 = get_int(data, 4)
                    clin_altura_tramo_3 = get_int(data, 6)
                    clin_ancho = get_int(data, 8)
                    clin_largo = get_int(data, 10)
                    clin_sector_0_x = get_int(data, 12)
                    clin_sector_0_y = get_int(data, 14)
                    clin_sector_1_x = get_int(data, 16)
                    clin_sector_1_y = get_int(data, 18)
                except Exception as e:
